...
const explosivelarge = extendContent(Block, "explosivelarge", {
  ...
  */
  ...
  mineTile(tile, other, dist){
    ...
    if(amount <= 0) return;
    var acceptTile = Units.findAllyTile(tile.getTeam(), tile.worldx(), tile.worldy(), 120, boolf(e=>(e.block().itemCapacity > 5 && !(e.block() instanceof Conveyor) && e.block().acceptStack(drops, amount, e, tile) >= 1)));
