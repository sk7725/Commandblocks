...
const accel = extendContent(Block, "accel", {
  ...
  draw(tile) {
    ...
    if(tile.rotation()%2==0) Draw.rect(this.topRegion, tile.drawx() + Math.max(0, (balltick-tile.ent().timer.getTime(ballid, 0))/3*tile.ent().getLast()), tile.drawy());
    else Draw.rect(this.topRegion, tile.drawx(), tile.drawy() + Math.max(0, (balltick-tile.ent().timer.getTime(ballid, 0))/3*tile.ent().getLast()));
  },
