const customtree=extend(FloatingDialog,{
    ...
    this.addListener(new ElementGestureListener(){
      zoom(event, initialDistance, distance){
        if(this.view.lastZoom < 0){
          this.view.lastZoom = this.view.getScaleX();
        }

        this.view.setScale(Mathf.clamp(distance / initialDistance * this.view.lastZoom, 0.25, 1));
        this.view.setOrigin(Align.center);
        this.view.setTransform(true);
      },
      ...
      pan(event, x, y, deltaX, deltaY){
        this.view.panX += deltaX / this.view.getScaleX();
        this.view.panY += deltaY / this.view.getScaleY();
